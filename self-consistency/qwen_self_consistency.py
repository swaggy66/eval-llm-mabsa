...
parser.add_argument("--test_lang", default='en', type=str, required=True,
                        help="The language of prompt, selected from: [en, fr, es, nl, ru]")
parser.add_argument("--temp", default=1, type=float, required=True,
                        help="temperature, selected from: [0.2, 0.4, 0.6, 0.8, 1]")
...
''' 
        ...
        generated_ids = model.generate(model_inputs.input_ids, max_new_tokens=512, do_sample=True, temperature=args.temp)
...
output_json_path = f"./results/qwen_consistency_results_{args.test_lang}_{args.temp}.json"

# Perform sentiment analysis
analyze_sentiments(file_path, output_json_path)

# python Self-Consistence/qwen_self_consistency.py  --lang en --test_lang [en,nl,es,ru,fr] --temp [0.2, 0.4, 0.6, 0.8, 1]