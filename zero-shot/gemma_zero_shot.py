...
def read_sentences_from_file(file_path):
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        ...
        if sentence:  # Add last sentence if file doesn't end with empty line
            sentences.append(" ".join([word.split()[0] for word in sentence]))
    return sentences

# Function to analyze sentiments and save results as JSON
...
# Specify the txt file path and analyze
file_path = f"./dataset/gold-{args.test_lang}-test.txt"
output_json_path = f"./results/gemma_zeroshot_results_{args.test_lang}.json"

analyze_sentiments(file_path, output_json_path)


# python Zero-Shot/gemma_zero_shot.py  --lang en --test_lang [en,nl,es,ru,fr]
