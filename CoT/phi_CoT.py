...
'''
        ...
        # employ pipeline
        outputs = chatbot(messages, max_new_tokens=520,do_sample=False,return_full_text=False,temperature=0.0)

        # extract text
        generated_text = outputs[0]["generated_text"]
        print(generated_text)
        #result_json = json.loads(generated_text.split('Output Format: ')[-1])  # 假设模型返回格式为可解析的JSON
...
output_file_path = f"./results/phi_CoT_{args.test_lang}_results.json"
analyze_sentiments(file_path, output_file_path)

# python CoT/phi_CoT.py  --lang en --test_lang [en,nl,es,ru,fr]
