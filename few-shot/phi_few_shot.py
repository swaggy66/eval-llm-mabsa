...
parser.add_argument("--topk", default=1, type=int, 
                    help="Number of top similar sentences to retrieve from training data, selected from: [1, 2, 4, 8, 16]")
...
def analyze_sentiments(test_file_path, train_file_path, output_json_path, top_k=1):
    ...
    for test_sentence, _ in tqdm(test_sentences):
...
train_file_path = f"./dataset/gold-{args.test_lang}-train.txt"
test_file_path = f"./dataset/gold-{args.test_lang}-test.txt"
output_json_path = f"./results/phi_fewshot_results_{args.test_lang}_{args.topk}.json"


analyze_sentiments(test_file_path, train_file_path, output_json_path, top_k=args.topk)

# python Few-Shot/phi_few_shot.py  --lang en --test_lang [en, fr, es, nl, ru] --topk [1, 2, 4, 8, 16]
