...
'''
        ...
        print('in-context:', similar_sentences_context)

   
        outputs = chatbot(messages, max_new_tokens=512, do_sample=False)
        generated_text = outputs[0]["generated_text"][1]["content"]

        generated_text = generated_text[len(messages):]
...
output_json_path = f"/data/zephyr_fewshot_results_{args.test_lang}.json"


analyze_sentiments(test_file_path, train_file_path, output_json_path, top_k=args.topk)


# python Few-Shot/zephyr_few_shot.py  --lang en --test_lang [en, fr, es, nl, ru] --topk [1, 2, 4, 8, 16]